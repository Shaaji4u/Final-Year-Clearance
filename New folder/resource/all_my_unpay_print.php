...
$pdf->SetSubject('My Unpaid Payment');

$pdf->SetKeywords('Linee, Classic, Nigeia, World, Fashion');
...
$html = '<table cellspacing="0" cellpadding="1" border="0" align="center">
    ...
    <tr>
       <td align="center" colspan="3" style="font-size:10;font-weight:bold;color:black">LIST OF ALL YOUR UNPAID PAYMENT AS AT - '.$dateprint_V.' </td>
    </tr>
...
$html1 ="";$status="0";
$stmt = $conn->prepare("SELECT  staff_work.work_id,staff_work.staff_id,staff_work.quantity,staff_work.sec_app,staff_work.date_secapp,work_booking.work_id,work_booking.price_quantity,work_booking.c_name,work_booking.sewing_cost FROM staff_work INNER JOIN work_booking ON  staff_work.work_id = work_booking.work_id  where staff_work.sec_app=? and staff_work.status=? and staff_work.withdraw=? and staff_work.staff_id=? ORDER BY  staff_work.date_secapp DESC, staff_work.staff_id ASC ");
	$stmt->execute(array("1",$status,$status,$_SESSION['staff_id']));
if ($stmt->rowCount () >= 1)
{

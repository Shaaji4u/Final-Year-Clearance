...
{
	var pas1 = document.getElementById("txtPasswordC1").value;
	var pas2 = document.getElementById("txtPasswordC2").value;
	
	if (pas1 != pas2) 
	{
		document.getElementById("result3").innerHTML="Confirmed Password not equall to real password";
		document.getElementById("result1").innerHTML="Confirmed Password not equall to real password";
	}
	else
	{
		if(pas1.length < 6){
			document.getElementById("result3").innerHTML="Password must be up to 6 or more characters !";
			document.getElementById("result1").innerHTML="Password must be up to 6 or more characters !";
		}else{
...
function check_good()
{
	var juu = document.getElementById('txtEmail').value;
	var goodmail=ValidateEmail(juu);
	if(goodmail == "wrong"){
		document.getElementById("result2").innerHTML="Error: Wrong Email Entered !";
	}else{
...
function stateChanged_dd() 
{ 
	if (xmlHttp.readyState==4)
	{ 
		var outcome = xmlHttp.responseText;
		if (outcome=="Exist"){
			document.getElementById('result2').innerHTML="Error: This Email is ALready Use by another staff to Register .";
		}else{
			var j ="2";
			wipeboxeror(j);
		}
...
function check_existing_email()
{
	...
	if (juu.length==0)
	  { 
		document.getElementById("result2").innerHTML="Error: Email box Can't be Empty";
		return;
	  }else{
			...
			if(goodmail != "wrong"){
				...
				var juu = byId("txtEmail").value;
				var url2="checkmailexist.php";
				parameters="email="+juu;
				xmlHttp.onreadystatechange=stateChanged_dd;
...
function check_password_empty(){
	var juu1 = byId("txtPasswordF").value;
	
	if(juu1.length <= 0){
		document.getElementById("result0").innerHTML="Current Password must not be Empty !"
	}
}
//wipe boxes
...
{ 
	...
	{ 
		...
		$txtQuantity=$txtStyleD=$txtDescription=$txtPerm=$cmblgov=$cmbstate=$txtGender=$txtPhone=$txtEmail=$txtFullname="";
		var outcome = xmlHttp.responseText;
		if(outcome!="No records"){
			var scat = outcome.split(";");
			//var t = scat[1];
				  
			byId('txtFullname').value = scat[0];
			byId('txtPhone').value = scat[1];
			byId('txtEmail').value = scat[2];
			byId('cmbstate').value = scat[3];
			byId('cmblgov').value = scat[4];
			byId('txtGender').value = scat[5];
			byId('txtPerm').value = scat[6];
			byId('txtDescription').value = scat[7];
			byId('txtStyleD').value = scat[8];
			byId('txtQuantity').value = scat[9];
		}else{
...
function retrieve_details(){
	var good_id =prompt("Please Enter the Id Of Your former Work Request ","");
